...
- Tools: `src/tools/document_search.py` (phase 1, FAISS), `web_search.py` (phase 2, `google_search` via an `AgentTool` sub-agent), `financial_data.py` (phase 3, MCP over Docker). Plain functions are auto-wrapped by ADK and their docstring *is* the tool description the LLM plans against - it is load-bearing, not decoration.
